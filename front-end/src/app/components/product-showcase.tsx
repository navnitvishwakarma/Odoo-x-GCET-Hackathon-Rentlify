...
interface Product {
  ...
  dailyPrice: number;
  weeklyPrice: number;
  salePrice: number; // Added for 'Buy' type
  condition: string;
}
...
}>(({ product, type = 'rent', onClick, onWishlistClick, onAddToCart }, ref) => {
...
export function ProductShowcase({ filters, type = 'rent', onProductClick, onWishlistClick, onAddToCart }: {
  filters: FilterState,
  type?: string,
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        setLoading(true);
        // data.data is the standard successful response structure based on previous analysis
        ...
        const mappedProducts = productsList.map((p: any) => ({
          ...
          // If backend doesn't have sale price, we mock it for demo if type is buy
          salePrice: p.pricing?.sale || p.pricing?.buy || (p.pricing?.daily * 30) || 0,
  ...
  const filteredProducts = products.filter((product) => {
    // Type Filter (Rent vs Buy)
    if (type === 'rent' && product.dailyPrice === 0) return false;
    // IF we are in Buy mode, we assume everything is buyable for now (since we mocked salePrice)
    // or we could filter: if (type === 'buy' && product.salePrice === 0) return false;
    ...
    // Price Filter
    // For rent, use dailyPrice. For buy, use salePrice.
    const priceToCheck = type === 'buy' ? product.salePrice : product.dailyPrice;
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-border pb-8">
          <div className="space-y-2">
            <h2 className="text-3xl md:text-4xl tracking-tight font-serif italic">
              {type === 'buy' ? 'Collection for Sale' : 'Rental Collection'}
            </h2>
